...
const isValidCommand = (step, command) => {
  ...
  if (isInvalidFirstCommand(step, command)) {
    console.error('The first command should be "PLACE <x-coordinate>,<y-coordinate>,<bearing>"');
    isValid = false;
  } else if (isInvalidKeyword(command)) {
    console.error('Please enter a valid command: PLACE, MOVE, LEFT, RIGHT, REPORT');
    isValid = false;
  }
...
const isValidCoordinate = coordinate => {
  ...
  if (!testX || !testY) {
    console.error(`OUT OF BOUND: Robot cannot be placed in this position [${coordinate[0]},${coordinate[1]}]`);
  }
  return testX && testY;
};
...
const isValidBearing = bearing => {
  ...
  if (!result) {
    console.error(`Please key in one of the valid bearings: NORTH, SOUTH, EAST, WEST`);
  }
  return result;
};

  
module.exports = { isInvalidFirstCommand, isInvalidKeyword, isCommandTypeOf, isValidCommand, isValidCoordinate, isValidBearing};